...
with col3:
    ...
    if st.session_state.phase == "BAN":
        ...
        if st.session_state.step < 10:
            ...
            if st.button("AI Gợi Ý"):
                ...
                with torch.no_grad():
                    for cand in available:
                        temp = [name_to_idx.get(n, 0) for n in st.session_state.final_draft]
                        temp[idx] = name_to_idx[cand]
                        x = torch.tensor(temp, dtype=torch.long)
                        p = model(x, edge_index, torch.tensor([0])).item()
                        scores.append((cand, p if is_blue else (1-p)))
                scores.sort(key=lambda x: x[1], reverse=True)
                for name, s in scores[:5]:
                    st.write(f"{name} ({s*100:.1f}%)")
                    st.progress(s)

            final_pick = st.selectbox("Xác nhận chọn:", ["-- Chọn --"] + available)
            ...
            if st.button("Reset"):
                for key in ['ban_list', 'final_draft', 'phase', 'step']: del st.session_state[key]
                st.rerun()