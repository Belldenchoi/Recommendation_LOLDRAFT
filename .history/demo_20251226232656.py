...
# --- Load Model & Assets ---
# --- Load Model & Assets (ĐÃ SỬA ĐỂ FIX BIAS TRIỆT ĐỂ) ---
@st.cache_resource
def load_assets():
    ...
    # =====================================================
    # ⚡ KỸ THUẬT HARD RESET: KHẮC PHỤC COLD START BIAS
    # =====================================================
    with torch.no_grad():
        # Bước A: Ép vector của "No Champion" (ID=0) về toàn số 0
        # Để đảm bảo: Không có tướng = Không có giá trị toán học
        # Giả sử ID 0 là No Champion (kiểm tra lại mapping của bạn nếu cần)
        no_champ_idx = 0 
        model.embedding.weight[no_champ_idx] = torch.zeros(32) # 32 là embedding_dim
        
        # Bước B: Ép Bias của lớp cuối cùng về 0
        # Để đảm bảo: Đầu vào là 0 thì đầu ra cũng là 0 (tức 50% sau Sigmoid)
        if hasattr(model.fc, 'bias'):
            model.fc.bias.fill_(0.0)

    model.eval()

    # Tạo cạnh đồ thị (Full connected)
    edges = []
    for i in range(10):
        for j in range(10):
