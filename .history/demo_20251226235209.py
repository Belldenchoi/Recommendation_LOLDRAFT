...
# --- HÀM RENDER GRID: CHỈ CÒN SEARCH & SCROLL ---
def render_champion_grid(champs_to_show, key_prefix, unique_id=0):
    """
    Hàm này bây giờ rất đơn giản:
    - Nhận vào danh sách tướng (đã được lọc ở bên ngoài).
    - Chỉ hiển thị Search Bar và Lưới ảnh.
    - Không còn nút lọc Radio/Tab nào cả.
    """
    
    # 1. SEARCH BAR (Tìm kiếm)
    c1, c2, c3 = st.columns([1, 4, 1])
    with c2:
        search_term = st.text_input(
            "🔍", # Icon search làm label cho gọn
    ...
    if not filtered:
        st.warning(f"Không tìm thấy tướng '{search_term}' trong danh sách này.")
        return None
...
with col_center:
    ...
    if st.session_state.phase == "BAN":
        ...
        # Ban thì cho hiện hết tướng (All) vì cấm con nào cũng được
    ...
    # >>> PHASE: PICK (CHỌN) <<<
        ...
        if st.session_state.step < 10:
            ...
            is_blue = idx < 5
            role_label = ROLE_NAMES[idx] # Ví dụ: "Top", "Jug"...
            ...
            # --- CHUẨN BỊ DANH SÁCH TƯỚNG (AUTO FILTER) ---
            # 1. Lấy danh sách tướng đúng Role hiện tại
            role_specific_champs = CHAMPION_ROLES.get(role_label, [])
            
            # 2. Lọc những con chưa bị pick/ban
            filtered_for_role = [c for c in available if c in role_specific_champs]
            
            # 3. Tạo tùy chọn "Off-meta" (Checkbox nhỏ)
            c_check, _ = st.columns([1, 1])
            with c_check:
                show_all = st.checkbox("Mở rộng (Hiện tướng khác Role)", value=False)
            
            # 4. Quyết định danh sách cuối cùng đưa vào Grid
            if show_all:
                final_list_to_show = available
            else:
                # Nếu lọc ra rỗng (trường hợp hiếm), fallback về available
                final_list_to_show = filtered_for_role if filtered_for_role else available

            # --- HIỂN THỊ GRID ---
            # Chỉ truyền danh sách đã lọc, Grid chỉ việc hiển thị
            user_pick = render_champion_grid(final_list_to_show, "pick_grid", unique_id=st.session_state.step)
