...
def render_champion_grid(available_champs, key_prefix, unique_id=0):
    """
    - unique_id: Để reset search box khi đổi lượt
    - Search Bar thay vì Filter Radio
    - Scroll Container
    """
    ...
    with c2:
        search_term = st.text_input(
            "🔍 Tìm tướng:", 
            placeholder="Nhập tên tướng (VD: Yasuo)...", 
    ...
    if search_term:
        ...
        filtered = [c for c in available_champs if search_term.lower() in c.lower()]
    else:
        filtered = available_champs
    
    if not filtered:
        st.warning(f"Không tìm thấy tướng nào có tên '{search_term}'")
        return None

    # 3. LƯỚI ẢNH TRONG KHUNG SCROLL (Cuộn dọc)
    # st.container(height=...) tạo ra vùng có thanh cuộn
    # height=450px là vừa đủ đẹp cho màn hình laptop
    with st.container(height=450, border=True):
        imgs = [get_champ_image(c) for c in filtered]
        
        # image_select nằm trọn trong container này
        idx = image_select(
            label="", 
            images=imgs, 
            captions=filtered, 
            use_container_width=False, # Để False để icon nhỏ gọn
...
with col_center:
    if st.session_state.phase == "BAN":
        ...
        if st.session_state.step < 10:
            ...
            # Grid Chọn (Search & Scroll)
            user_pick = render_champion_grid(available, "pick_grid", unique_id=st.session_state.step)
