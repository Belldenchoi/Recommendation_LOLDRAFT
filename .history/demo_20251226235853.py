...
# --- HÀM RENDER GRID (ĐÃ TỐI ƯU CHO CỘT GIỮA) ---
def render_champion_grid(available_champs, key_prefix, default_filter="All"):
    # 1. Bộ lọc Role (Ngang)
    role_map = {"ALL": "All", "TOP": "Top", "JUNGLE": "Jug", "MID": "Mid", "ADC": "Adc", "SUP": "Sup"}
    
    # Hiển thị Radio dạng ngang
    selected_label = st.radio("Lọc vị trí:", list(role_map.keys()), horizontal=True, label_visibility="collapsed", key=f"{key_prefix}_filter", index=0 if default_filter=="All" else list(role_map.values()).index(default_filter))
    
    role_key = role_map[selected_label]
    if role_key == "All": filtered = available_champs
    else: filtered = [c for c in available_champs if c in CHAMPION_ROLES.get(role_key, [])]
    
    if not filtered:
        st.warning("Không có tướng phù hợp!")
        return None

    # 2. Lưới ảnh
    imgs = [get_champ_image(c) for c in filtered]
    idx = image_select(label="", images=imgs, captions=filtered, use_container_width=True, key=f"{key_prefix}_select", return_value="index")
...
# Chia layout: 1 (Blue) - 2 (Center/Action) - 1 (Red)
# Cột giữa rộng gấp đôi để hiển thị Grid ảnh cho thoải mái
col_blue, col_center, col_red = st.columns([1.2, 2.5, 1.2])
...
if all(x is None for x in st.session_state.final_draft):
    ...
    blue_wr_val = get_current_win_rate()
...
with col_blue:
    ...
    # Hiển thị Ban của Blue (0, 2, 4, 6, 8)
    st.caption("Bans:")
    bans_blue = st.columns(5)
    for i in range(5):
        ban_idx = i * 2
        with bans_blue[i]:
            if ban_idx < len(st.session_state.ban_list):
                st.image(get_champ_image(st.session_state.ban_list[ban_idx]), use_container_width=True)
            else:
                st.image("https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/champion-icons/-1.png", use_container_width=True)
...
with col_red:
    ...
    # Hiển thị Ban của Red (1, 3, 5, 7, 9)
    st.caption("Bans:")
...
# --- CỘT GIỮA: KHU VỰC THAO TÁC (ACTION CENTER) ---
with col_center:
    # 1. TRẠNG THÁI (HEADER)
    if st.session_state.phase == "BAN":
        ...
        # Grid Cấm
        ban_pick = render_champion_grid(available, "ban_grid", "All")
        ...
        if st.session_state.step < 10:
            ...
            # AI Suggestion (Expander cho gọn)
            ...
            # Grid Chọn (Mặc định lọc theo Role hiện tại)
            user_pick = render_champion_grid(available, "pick_grid", default_filter=role_label)
