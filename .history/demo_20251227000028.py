...
with col_center:
    ...
    if st.session_state.phase == "BAN":
        ...
        # Grid Ban: Hiện tất cả tướng (available)
    ...
    # >>> PICK PHASE <<<
        ...
        if st.session_state.step < 10:
            ...
            with st.expander("🤖 Gợi ý từ AI (Đã lọc theo vị trí)", expanded=True):
                if st.button("💡 Phân tích & Gợi ý"):
                    with st.spinner("AI đang tính toán..."):
                        ...
                        # 1. LẤY DANH SÁCH TƯỚNG ĐÚNG ROLE
                        role_cands = CHAMPION_ROLES.get(role_label, [])
                        
                        # 2. CHỈ TÌM TRONG DANH SÁCH ĐÓ (Strict Filtering)
                        # Nếu role_cands rỗng (file lỗi) thì mới fallback, còn không thì chỉ tìm trong role_cands
                        if role_cands:
                            search_space_ai = [c for c in available if c in role_cands]
                        else:
                            search_space_ai = available # Chỉ fallback khi data lỗi
                            
                        base = 0.5 if all(x is None for x in st.session_state.final_draft) else get_current_win_rate()
                        
                        if not search_space_ai:
                            st.warning(f"Đã hết tướng cho vị trí {role_label}!")
                        else:
            ...
            # Logic hiển thị: Nếu tick mở rộng thì hiện all, không thì hiện đúng role
            if show_all:
                final_list_to_show = available
            else:
                # Nếu lọc ra rỗng (hiếm), fallback về available để không bị trắng bảng
