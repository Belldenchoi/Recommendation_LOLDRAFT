...
def load_champion_roles_from_csv():
    ...
    try:
        pd.read_csv("data/champ_data.csv")
...
with col_center:
    ...
    # >>> PHASE: PICK <<<
        ...
        if st.session_state.step < 10:
            ...
            # AI SUGGESTION
            with st.expander("🤖 Gợi ý từ AI", expanded=True):
                if st.button("💡 Phân tích & Gợi ý"):
                    with st.spinner("AI đang tính toán..."):
                        ...
                        if not search_space_ai:
                            ...
                            # Logic tính toán: Chỉ cần sort theo score của Model
                            # Blue muốn score cao (gần 1), Red muốn score thấp (gần 0)
                            for cand in search_space_ai:
                                ...
                                # Lấy score trực tiếp từ model
                                score = model(torch.tensor(ids), edge_index, torch.zeros(10, dtype=torch.long)).item()
                                suggestions.append((cand, score))
                            
                            # Nếu là Blue thì sort giảm dần (Cao nhất lên đầu)
                            # Nếu là Red thì sort tăng dần (Thấp nhất lên đầu - tức là Blue thua)
                            ...
                            for i, (name, score) in enumerate(suggestions[:5]):
                                with cols[i]:
                                    ...
                                    # Chỉ hiện tên, KHÔNG hiện % winrate
                                    st.caption(f"**{name}**")
