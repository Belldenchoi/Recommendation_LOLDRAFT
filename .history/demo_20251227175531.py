...
with col_center:
    ...
    # >>> PHASE: PICK <<<
        ...
        if st.session_state.step < 10:
           ...
           # AI SUGGESTION (FIXED FOR RED TEAM)
            with st.expander("🤖 Gợi ý từ AI (Phân tích Tác Động thực tế)", expanded=True):
                if st.button("💡 Phân tích & Gợi ý"):
                    progress_text = "AI đang tính toán tác động..."
                    ...
                    if not search_space_ai:
                        ...
                        # 1. TÍNH BASE SCORE (Tỷ lệ thắng hiện tại trước khi pick slot này)
                        # Tạo bản nháp, đảm bảo vị trí đang chọn (idx) là Trống (0) để làm mốc so sánh
                        base_draft = st.session_state.final_draft.copy()
                        base_draft[idx] = None 
                        ids_base = [name_to_idx.get(n if n else "No Champion", 0) for n in base_draft]
                        
                        with torch.no_grad():
                            # Đây là tỷ lệ thắng của Blue hiện tại
                        ...
                        for idx_prog, cand in enumerate(search_space_ai):
                            ...
                            # 2. TÍNH NEW SCORE (Tỷ lệ thắng sau khi thử pick tướng này)
                            tmp_draft = st.session_state.final_draft.copy()
                            tmp_draft[idx] = cand
                            ids_new = [name_to_idx.get(n if n else "No Champion", 0) for n in tmp_draft]
                            
                            with torch.no_grad():
                                new_blue_wr = model(torch.tensor(ids_new), edge_index, torch.zeros(10, dtype=torch.long)).item()
                            
                            # 3. TÍNH DELTA (SỰ THAY ĐỔI)
                            # Raw Delta luôn là: Mới - Cũ
                            raw_delta = new_blue_wr - base_blue_wr
                            
                            # 4. CHUẨN HÓA GÓC NHÌN (QUAN TRỌNG)
                            if is_blue:
                                # Blue muốn Blue WR tăng -> Delta dương là Tốt
                                impact = raw_delta
                                final_score = new_blue_wr # Dùng để sort
                            else:
                                # Red muốn Blue WR GIẢM -> Delta âm là Tốt
                                # Nên ta đảo dấu: Blue giảm 5% (-0.05) tức là Red lợi 5% (+0.05)
                                impact = -raw_delta 
                                final_score = 1.0 - new_blue_wr # Red WR
                            
                            # Cộng thêm điểm impact * 10 để AI ưu tiên những con làm thay đổi cục diện trận đấu
                            ranking_score = final_score + (impact * 10.0)
                            
                            suggestions.append((cand, ranking_score, impact))
                        
                        my_bar.empty()
                        
                        # Sort giảm dần (Impact càng dương càng tốt cho phe đang chọn)
                        ...
                        with st.container(height=500, border=True):
                            ...
                            for i in range(0, len(suggestions), cols_per_row):
                                ...
                                for j, (name, r_score, imp) in enumerate(row_cands):
                                    with cols[j]:
                                        ...
                                        # Hiển thị Impact
                                        imp_percent = imp * 100
                                        # Ngưỡng nhỏ 0.1% để hiển thị màu
                                        if imp_percent > 0.1:
                                            st.markdown(f"<div style='text-align:center; color:#00cc00; font-size:11px;'>▲ +{imp_percent:.1f}%</div>", unsafe_allow_html=True)
                                        elif imp_percent < -0.1:
                                            # Nếu Impact âm tức là pick vào làm team yếu đi
                                            st.markdown(f"<div style='text-align:center; color:#ff3333; font-size:11px;'>▼ {imp_percent:.1f}%</div>", unsafe_allow_html=True)
                                        else:
