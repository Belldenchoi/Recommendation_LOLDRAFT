...
# >>> TRANG 2: MODEL ANALYTICS (MỚI) <<<
    ...
    # 3. Model Parameters
 def render_tsne_chart():
    """Hiển thị ảnh t-SNE đã được vẽ sẵn từ file draw_tnse.py"""
    image_path = "champion_embeddings_tsne.png"  # Tên file ảnh bạn đã lưu
    
    if os.path.exists(image_path):
        st.image(image_path, caption="Biểu đồ t-SNE (Đã được Pre-render)", use_container_width=True)
    else:
        st.error(f"⚠️ Không tìm thấy file '{image_path}'. Vui lòng chạy file 'draw_tnse.py' trước để tạo ảnh!")